...
import com.twb.dadale.manager.AliyunAiManager;
...
public class QuestionController {
    ...
    @Resource
    private AiManager aiManager;  // ChatGLM

    @Resource
    private AliyunAiManager aliyunAiManager; // 阿里百炼
    ...
    @PostMapping("/ali_generate")
    public BaseResponse<List<QuestionContentDTO>> aliGenerateQuestion(
            @RequestBody AiGenerateQuestionRequest aiGenerateQuestionRequest) {
        ThrowUtils.throwIf(aiGenerateQuestionRequest == null, ErrorCode.PARAMS_ERROR);
        // 获取参数
        Long appId = aiGenerateQuestionRequest.getAppId();
        ...
        String userMessage = getGenerateQuestionUserMessage(app, questionNumber, optionNumber);
        // AI 生成
        String result = aliyunAiManager.doSyncRequest(GENERATE_QUESTION_SYSTEM_MESSAGE, userMessage, 0.7f);
        // 截取需要的 JSON 信息
        int start = result.indexOf("[");
        int end = result.lastIndexOf("]");
        String json = result.substring(start, end + 1);
